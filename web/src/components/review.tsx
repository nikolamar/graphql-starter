...
export const Review: FC<ReviewListItemProps> = ({ children: review }) => {
  ...
  const handleVote = (value: 1 | -1) => {
    vote({
      variables: { reviewId: review.id, value },
      update: (cache) => {
  ...
  return (
    <Box mb={defaults.margin}>
      <HStack align="flex-start">
        <Avatar
          name={review.user?.profile.fullName || undefined}
          src={review.user?.profile.image?.url || undefined}
        />
        <VStack align="flex-start" spacing={1}>
          ...
          <VStack
            ...
            bgColor={color}
          >
            <Text mb={defaults.margin} textAlign="justify" fontSize={14}>
              {review.message}
            </Text>
            <HStack spacing={1}>
          ...
          <HStack pl={2}>
            <Link>
              <Text fontSize={10} onClick={() => handleVote(1)}>
                Like
              </Text>
            </Link>
            <Link>
